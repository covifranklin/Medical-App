...
function shimmer() {
  return "animate-pulse bg-warm-200 rounded";
}
...
export function SkeletonCard({ lines = 3 }: { lines?: number }) {
  return (
    <div className="rounded-2xl border border-warm-200 bg-white p-4 shadow-soft space-y-3">
  ...
  return (
    <div className="space-y-3">
      {Array.from({ length: count }).map((_, i) => (
        <div key={i} className="rounded-2xl border border-warm-200 bg-white p-4 shadow-soft">
      ...
      <div>
        ...
        <div className="mt-3 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {Array.from({ length: 3 }).map((_, i) => (
            <div key={i} className="rounded-2xl border border-warm-200 bg-white p-4 shadow-soft space-y-3">
  ...
  return (
    <div className="space-y-8">
      {Array.from({ length: 3 }).map((_, i) => (
        <div key={i}>
          <SkeletonLine width="w-48" height="h-5" />
          <div className={`mt-3 rounded-2xl border border-warm-200 bg-white p-4 ${shimmer()}`} style={{ height: i === 0 ? 220 : 200 }} />
        </div>
      ))}
    </div>
  );
