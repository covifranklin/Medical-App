...
const CONFIDENCE_BADGE: Record<string, { className: string; label: string }> = {
  low: { className: "bg-gray-100 text-gray-700", label: "Low confidence" },
  medium: { className: "bg-blue-100 text-blue-700", label: "Medium confidence" },
...
function ExerciseReviewCard({
  ...
  return (
    <div className={`rounded-md border ${style.border} ${style.bg} p-3`}>
      ...
      <p className="mt-1 text-sm text-gray-700">{review.feedback}</p>
...
export default function AIReviewDisplay({
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Overall assessment */}
      <div>
        <p className="text-sm text-gray-900">{result.overallAssessment}</p>
      </div>

      {/* Exercise reviews */}
      {result.exerciseReviews && result.exerciseReviews.length > 0 && (
        <div>
          <h3 className="text-sm font-semibold text-gray-900 mb-2">Exercise Reviews</h3>
      ...
      {result.crossConditionWarnings && result.crossConditionWarnings.length > 0 && (
        <div>
          <h3 className="text-sm font-semibold text-gray-900 mb-2">Cross-Condition Warnings</h3>
          <ul className="space-y-1.5">
            {result.crossConditionWarnings.map((warning, i) => (
              <li key={i} className="flex gap-2 rounded-md border border-red-200 bg-red-50 px-3 py-2">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {result.strengths && result.strengths.length > 0 && (
          <div>
            <h3 className="text-sm font-semibold text-gray-900 mb-1.5">Strengths</h3>
        ...
        {result.concerns && result.concerns.length > 0 && (
          <div>
            <h3 className="text-sm font-semibold text-gray-900 mb-1.5">Concerns</h3>
            <ul className="space-y-1">
      ...
      {result.suggestions && result.suggestions.length > 0 && (
        <div>
          <h3 className="text-sm font-semibold text-gray-900 mb-1.5">Suggestions</h3>
          <ul className="space-y-1">
